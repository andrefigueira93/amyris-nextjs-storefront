...
export async function getProductsInCollection() {
  ...
  const allProducts = data.collectionByHandle.products.edges
...
export async function getAllProducts() {
  ...
  const slugs = data.products.edges ? data.products.edges : [];
...
export async function recursiveCatalog(cursor = '', initialRequest = true) {
  ...
  if (cursor !== '') {
    ...
    const { data } = await ShopifyData(query);
    data = data.products.edges ? data.products.edges : [];

    if (data.products.pageInfo.hasNextPage) {
      const num = data.products.edges.length;
      const cursor = data.products.edges[num - 1].cursor;
